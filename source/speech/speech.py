...
def spellTextInfo(
	...
	"""Spells the text from the given TextInfo, honouring any LangChangeCommand objects it finds if
	autoLanguageSwitching is enabled."""
...
def getCharDescListFromText(text, locale):
	...
	is made up of, by checking the presence of character descriptions in characterDescriptions.dic of that
...
def getObjectPropertiesSpeech(  # noqa: C901
	...
	for name, value in allowedProperties.items():
		if name == "includeTableCellCoords":
			...
			# Certain properties such as row and column numbers have presentational versions, which should be
			# used for speech if they are available.
			# Therefore redirect to those values first if they are available, falling
			# back to the normal properties if not.
			names = [name]
			if name == 'rowNumber':
	...
	# When speaking an object due to a focus change, the 'selected' state should not be reported if only one
	# item is selected.
	...
	if states is not None and reason == OutputReason.FOCUS:
		if (
			...
			# We must copy the states set and  put it back in newPropertyValues otherwise mutating the
			# original states set in-place will wrongly change the cached states.
			# This would then cause 'selected' to be announced as a change when any
			# other state happens to change on this object in future.
			states = states.copy()
...
def _getPlaceholderSpeechIfTextEmpty(
	...
	@return: (True, SpeechSequence) if text for obj was considered empty and we attempted to get speech for
	the placeholder value. (False, []) if text for obj was not considered empty.
	"""
...
def getIndentationSpeech(indentation: str, formatConfig: Dict[str, bool]) -> SpeechSequence:
	...
	if toneIndentConfig:
		if quarterTones <= IDT_MAX_SPACES:
			...
			# we have more than 72 spaces (18 tabs), and must speak it since we don't
			# want to hurt the users ears.
			speak = True
	if speak:
	...
	for item in oldSpeechSequence:
		if isinstance(item, LangChangeCommand):
			...
			if not curLanguage or (
				not autoDialectSwitching and curLanguage.split('_')
				[0] == defaultLanguageRoot
			):
...
def speakSelectionChange(  # noqa: C901
	...
	@param generalize: if True, then this function knows that the text may have changed between the creation
	of the oldInfo and newInfo objects, meaning that changes need to be spoken more generally, rather than
	speaking the specific text, as the bounds may be all wrong.
	@param priority: The speech priority.
	"""
	selectedTextList = []
	unselectedTextList = []
	...
	if speakUnselected:
		if not generalize:
			...
			if not newInfo.isCollapsed:
				...
				if len(text) == 1:
					text = characterProcessing.processSpeechSymbol(locale, text)
				# Translators: This is spoken to indicate when the previous selection was
				# removed and a new selection was made. for example 'hello world selected
				# instead'
...
class SpeakTextInfoState(object):
	"""Caches the state of speakTextInfo such as the current controlField stack, current formatfield and
	indentation."""
...
def getTextInfoSpeech(  # noqa: C901
	...
	reportIndentation = unit == textInfos.UNIT_LINE and (
		formatConfig["reportLineIndentation"] or formatConfig["reportLineIndentationWithTones"])
	# For performance reasons, when navigating by paragraph or table cell,
	# spelling errors will not be announced.
	...
	for count in range(min(len(newControlFieldStack), len(controlFieldStackCache))):
		# #2199: When comparing controlFields try using uniqueID if it exists before resorting to compairing
		# the entire dictionary
		oldUniqueID = controlFieldStackCache[count].get('uniqueID')
		newUniqueID = newControlFieldStack[count].get('uniqueID')
		if (
			(oldUniqueID is not None or newUniqueID is not None)
			and newUniqueID == oldUniqueID
	...
	# We don't do this for focus because hearing "out of list", etc. isn't
	# useful when tabbing or using quick navigation and makes navigation less
	# efficient.
	...
	# When true, we are inside a clickable field, and should therefore not
	# announce any more new clickable fields
	inClickable = False
	...
	for count in range(commonFieldCount, len(newControlFieldStack)):
		...
		if not inClickable and formatConfig['reportClickable']:
			...
			if states and controlTypes.STATE_CLICKABLE in states:
				# We entered the most outer clickable, so announce it, if we won't be
				# announcing anything else interesting for this field
	...
	# Similar to before, but If the most inner clickable is exited, then we
	# allow announcing clickable for the next lot of clickable fields entered.
	inClickable = False
	# Move through the field commands, getting speech text for all controlStarts, controlEnds and formatChange
	# commands
			...
			if command.command == "controlStart":
				...
				if not inClickable and formatConfig['reportClickable']:
					...
					if states and controlTypes.STATE_CLICKABLE in states:
						...
						# Announce it if there is nothing else interesting about the field, but
						# not if the user turned it off.
						presCat = command.field.getPresentationCategory(
							newControlFieldStack[0:], formatConfig, reason)
	...
	# that are common with the old controlFieldStack (for closing), if extra
...
def getPropertiesSpeech(  # noqa: C901
		...
		if rowNumber and (not sameTable or rowNumber != oldRowNumber or rowSpan != oldRowSpan):
			...
			if includeTableCellCoords and not cellCoordsText:
				...
				if rowSpan > 1 and columnSpan <= 1:
					# Translators: Speaks the row span added to the current row number
					# (example output: through 5).
		...
		if columnNumber and (not sameTable or columnNumber != oldColumnNumber or columnSpan != oldColumnSpan):
			...
			if includeTableCellCoords and not cellCoordsText:
				...
				if columnSpan > 1 and rowSpan <= 1:
					# Translators: Speaks the column span added to the current column number
					# (example output: through 5).
	...
	if rowCount and columnCount:
		# Translators: Speaks number of columns and rows in a table (example
		# output: with 3 rows and 2 columns).
	...
	if rowCount or columnCount:
		# The caller is entering a table, so ensure that it is treated as a new
		# table, even if the previous table was the same.
		oldTableID = None
...
def getControlFieldSpeech(  # noqa: C901
	...
	if(
		...
		getProps = {
			'rowNumber': (
				attrs.get("table-rownumber-presentational") or attrs.get("table-rownumber")),
			'columnNumber': (
	...
	if (
		...
		# element.
		if (
			# Don't speak name when labelledByContent. It will be spoken by the
			# subsequent controlFields instead.
...
def getFormatFieldSpeech(  # noqa: C901
	...
	if formatConfig["reportAlignment"]:
		...
		if (verticalAlign or oldverticalAlign is not None) and verticalAlign != oldverticalAlign:
			...
			if verticalAlign == "top":
				...
				# Translators: Reported when text is vertically justified but with
				# character spacing (For some Asian content).
	...
	if formatConfig["reportSpellingErrors"]:
		...
		if (invalidGrammar or oldInvalidGrammar is not None) and invalidGrammar != oldInvalidGrammar:
			...
			if text:
				textList.append(text)
	# The line-prefix formatField attribute contains the text for a bullet or number for a list item, when
	# the bullet or number does not appear in the actual text content.
	# Normally this attribute could be repeated across formatFields within a list item and therefore is not
	# safe to speak when the unit is word or character.
	# However, some implementations (such as MS Word with UIA) do limit its useage to the very first
	# formatField of the list item.
...
def getTableInfoSpeech(
	...
	if newTable:
		...
		# Translators: reports number of columns and rows in a table (example
		# output: table with 3 columns and 5 rows).
		text = _("table with {columnCount} columns and {rowCount} rows").format(
